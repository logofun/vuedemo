          ...
          <a-col :key="index" :span="6" v-if="isUpStatus ? (isUpStatus && (index < 3)) : true">
            <a-form-item :label="item.label" :labelCol="{ span: 8}" :wrapperCol="{ span: 16 }" :style="{ width: '100%', marginBottom: '8px' }">
              ...
              <template v-if="item.type === 'select'">
                <a-select v-bind="{...item.props}" v-model:value="formData[item.key]" />
              </template>
              <template v-if="item.type === 'datePicker'">
                <a-date-picker v-bind="{...item.props}" v-model:value="formData[item.key]" style="width: 100%" />
              </template>
              <template v-if="item.type === 'rangePicker'">
                <a-range-picker v-bind="{...item.props}" v-model:value="formData[item.key]" style="width: 100%" />
              </template>
            </a-form-item>
          </a-col>
        </template>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const count = computed(() => Object.keys(renderForm).length < 4 ? computeSpan(Object.keys(renderForm).length) : 6)
